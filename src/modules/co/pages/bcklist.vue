<template>
    ...
    <v-card width="100%" height="100%" elevation="10" class="rounded-lg">
      ...
      <v-card outlined class="mt-2"> 
        <v-card-text>   
           <div id="chart2">
            <apexchart  ref="myChart2" height="550" 
    ...
    <v-dialog v-model="dialogDetail">
        <v-card class="mx-auto">
          <v-card-title>
              Cabang: {{ this.cabangdetail }} ({{  this.tanggal_detail|formatTglIndo }})
          </v-card-title>
          <v-card-subtitle>Masalah : {{ this.masalahdetail }}</v-card-subtitle>
          <v-card-text>
             <ve-table 
                fixed-header
                :border-x="true" 
  ...
  computed: {
    ...
    chartOptions2() {
        return ({chart: { 
            ...
            title: {
              text: 'Trending Complaint Online',
              align: 'center',
               style: {
                fontSize:  '18px',
                fontWeight:  'bold', 
            ...
            subtitle: {
                ...
                text: "Periode : " +  moment(String(this.dateAwal)).format('DD MMM YYYY') + " s/d " + moment(String(this.dateAkh)).format('DD MMM YYYY'),
                align: 'center',
                style: {
                   floating: true
                },
      ...
      detailCabang (tanggal) {
          ...
          this.$store.dispatch("getPerCabang", dataPercabang).then( () =>{  
            ...
            var a = [{
                    ...
                    fixed: "left",
                    type: "expand"
                },{field: "TANGGAL" , key: "TANGGAL", title: "TANGGAL", width: 125, align: "center",fixed: "left"},
