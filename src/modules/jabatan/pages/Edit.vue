...
<v-container fluid>
  ...
  >
    ...
    <v-card-title>Edit Data Jabatan </v-card-title>
          ...
          <v-row dense>
            <v-col cols="12" lg="4">
               ...
               class="ma-1 pa-1 mx-4"
                ...
                label="Nama Jabatan" 
                :rules="[v => !!v || 'Nama Jabatan Harus diisi']"
              ></v-text-field> 
                <v-btn small color="teal accent-3" class="d-flex justify-end mb-6" @click="saveJabatan">
                  Simpan
                </v-btn>
              </v-col>
          </v-row>
  ...
  methods: {
    saveJabatan() {
      if(this.$refs.form.validate()){
        ...
        this.$store
          .dispatch("simpanUpdateJabatan", datanya)
          .then(() => this.$router.push("/jabatan/list"))
          .catch((err) => {
    ...
    getData() {
        this.$store.dispatch("getOneJabatan", {id: this.$route.params.id})
          .then(response => {
            let n = response[0]
            this.nama_jabatan = n.nama_jabatan
          })
          .catch(e => {
            console.log(e);
          });
