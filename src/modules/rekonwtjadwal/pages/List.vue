          ...
          <v-row dense> 
            <v-col cols="5"> 
               <v-select
                    ...
                    v-model="listCabangRekonwtJadwal"
                  ...
                  >
                    ...
                    >
                      <v-list-item-action>
                        <v-icon :color="listCabangRekonwtJadwal.length > 0 ? 'indigo darken-4' : ''">
                            {{ iconCabang }}
                          </v-icon>
                      </v-list-item-action>
                      <v-list-item-content>
...
export default { 
  ...
  computed: { 
    ...
    listdata() { 
      return this.$store.getters.detailRekonwtJadwal;
    },   
    getCabang() {
      return this.$store.getters.listCabangRekonwtJadwal;
    }, 
    pilihAllCabang () {
        return this.listCabangRekonwtJadwal.length === this.$store.getters.listCabangRekonwtJadwal.length
    },
    pilihSomeCabang () {
        return this.listCabangRekonwtJadwal.length > 0 && !this.$store.getters.listCabangRekonwtJadwal.length
      },
    iconCabang () { 
  ...
  methods: { 
      saveJadwal() {
        ...
        if(this.$refs.form.validate()){
            const a = this.listCabangRekonwtJadwal.map((item)=>{ 
          ...
          this.$store
            .dispatch("simpanAddRekonwtJadwal", datanya)
            .then(() => {

              this.tanggal1=""
              this.tanggal2="" 
              this.listCabangRekonwtJadwal = [] 
              this.loaddata()  
            ...
            .catch(() => {
              this.pesan = "Error Setting Jadwal";
              this.snackbar = true;
              this.overlay = false
            });
        }else{
      ...
      toggle () {
          this.$nextTick(() => {
            if (this.pilihAllCabang) {
              ...
              this.listCabangRekonwtJadwal = this.$store.getters.listCabangRekonwtJadwal.slice()
            }
          })
      },          
      loaddata(){ 
        ...
        this.$store.dispatch("getRekonwtJadwal", datanya).then(()=>{
          this.overlay = false
        }).catch(() =>{
           this.overlay = false
        })
  ...
  mounted() {   
    ...
    this.loaddata() 
    this.$store.dispatch("getCabangRekonwtJadwal", { kdcab : dataUser.cover});
  },
};
</script>
