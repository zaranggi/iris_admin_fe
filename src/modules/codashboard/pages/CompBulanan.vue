          ...
          <v-card-text>    
                  <v-row dense>
                    <v-col cols="4">
                      ...
                      @click.stop="showDetailCOKategori('Sesuai SLA')">
                          <v-list-item two-line>
                            <v-list-item-content>
                              <v-list-item-title class="headline white--text"> Sesuai SLA </v-list-item-title>
                            </v-list-item-content>
                          </v-list-item>
                          <v-card-text>
                            <v-row align="center">
                    ...
                    <v-col cols="4">
                      ...
                      @click.stop="showDetailCOKategori('Lewat SLA')">
                          <v-list-item two-line>
                            <v-list-item-content>
                              <v-list-item-title class="headline white--text"> Lewat SLA </v-list-item-title> 
                            </v-list-item-content>
                          </v-list-item>
                          <v-card-text>
                            <v-row align="center">
...
export default { 
  data() {
    return { 
      ...
      tanggal: "",
      itemshift:[ { nama_jenis : 'Semua Shift', id: 'All'}
            ],  
      headers:[],  
       headersDetail:[
  ...
  computed: { 
    ...
    catatanDashboard() { 
      return this.$store.getters.catatanDashboardBulanan;
    },     
    listdata() { 
      return this.$store.getters.codashboardTableBulanan;
    },   
    listDetailCo() { 
      return this.$store.getters.detailcoDashboardBulanan;
    }, 
    t_sla() { 
      return this.$store.getters.t_sla;
    },    
    t_lsla() { 
      return this.$store.getters.t_lsla;
    },    
    t_co_bulanan() { 
      return this.$store.getters.t_co_bulanan;
    },    
    
    footerDataDasboard() { 
      return this.$store.getters.footerDataDasboardBulanan;
    }, 
    getCabang() {
  ...
  methods: { 
      ...
      handleSubmitCo(){
         if(this.shift.length === 0){
            ...
            this.$store.dispatch("getCoDashboardBulanan", datanya).then( () =>{   
                var a = [{field: "TANGGAL" , key: "TANGGAL", title: "PERIODE", width: 125, align: "center",fixed: "left"},
      ...
      loadawal() {
          ...
          this.$store.dispatch("getCabangCoDashboard", { kdcab : dataUser.cover}).then(() =>{
                ...
                this.$store.dispatch("getCoDashboardBulanan", datanya).then( () =>{   
                  var ha = [{field: "TANGGAL" , key: "TANGGAL", title: "PERIODE", width: 100, align: "center",fixed: "left"},
    ...
    showDetailCO(tanggal,cabang,masalah) {
      ...
      this.$store.dispatch("getDetailCoDashboardBulanan", payload).then( () =>{  
    ...
    showDetailCOKategori(status_co) {
      ...
      this.$store.dispatch("getDetailCoDashboardStatusCoBulanan", payload).then( () =>{  
        this.dialogDetail = !this.dialogDetail
        loading.hide()
      }).catch(()=>{
        loading.hide()
