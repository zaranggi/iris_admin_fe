<template>
    <v-card width="100%" height="100%" elevation="10" class="rounded-lg" >
      <v-card-title primary-title> List Stockout Tidak Sesuai </v-card-title>
      <v-divider />  
      <v-card-text> 
        <div class="d-flex flex-column ma-4">
          <v-form @submit.prevent="handleSubmitSto">
              ...
              <v-row dense>
                <v-col cols="12" lg="6">
                  ...
                  <template v-slot:prepend-item>
                    ...
                    >
                      <v-list-item-action>
                        <v-icon :color="listCabangSto.length > 0 ? 'indigo darken-4' : ''">
                            {{ iconCabang }}
                          </v-icon>
                      </v-list-item-action>
                      <v-list-item-content>
...
export default {
  data() {
    return {
      ...
      search: "",
      overlay: true,
      dateAwalSto: new Date().toISOString().substr(0, 10),
      tanggal: false,
      listCabangSto: "",
  ...
  computed: { 
    computeTanggal() {
      return this.dateAwalSto;
    },
    loading() {
      return this.$store.getters.loadingsto;
    },   
    listdata() { 
      return this.$store.getters.listData;
    },  
    getCabang() {
      return this.$store.getters.listCabangSto;
    },
    isMobile() {
      return this.windowWidth <= 300
    },
    pilihAllCabang () {
        return this.listCabangSto.length === this.$store.getters.listCabangSto.length
    },
    pilihSomeCabang () {
        return this.listCabangSto.length > 0 && !this.$store.getters.listCabangSto.length
      },
    iconCabang () { 
  ...
  methods: {  
      handleSubmitSto(){
        this.overlay = true
        const a = this.listCabangSto.map((item)=>{ 
          return item.id === undefined ? item : item.id
        })  
        
         const datanya = {
            kdcab: a.toString(),
            tanggal: this.dateAwalSto,
        };
        this.$store.dispatch("getSto", datanya).then(()=> {
          this.overlay = false
        }).catch(() => {
          this.overlay = false
        })
      ...
      toggle () {
          this.$nextTick(() => {
            if (this.pilihAllCabang) {
              ...
              this.listCabangSto = this.$store.getters.listCabangSto.slice()
            }
          })
        },
  ...
  mounted() {   
    ...
    this.$store.dispatch("getCabangUser", { kdcab : dataUser.cover});
    this.overlay = false
