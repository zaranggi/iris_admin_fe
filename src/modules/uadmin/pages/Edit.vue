...
<v-container fluid>
  ...
  >
    ...
    <v-card-title>Edit User |  {{this.nama}}</v-card-title>
  ...
  methods: {
    saveUadmin() {
      ...
      if(this.$refs.form.validate()){
        ...
        flag_gabung: this.flag_gabung === true ? 'Y' : 'N',
        ismobile: this.ismobile === true ? 'Y' : 'N',
        };
        this.$store
          .dispatch("simpanUpdate", datanya)
          .then(() => this.$router.push("/uadmin/list"))
          .catch((err) => {
      ...
      this.overlay = true
        this.$store.dispatch("getOneUser", {id: this.$route.params.id})
          .then(response => { 
            ...
            this.nik = n.nik
            this.nama = n.nama 
            this.id_chat = n.id_chat 
            this.listCabang = n.kdcab
            this.listDep = n.id_dep 
            this.listJabatan = n.id_jabatan
            if(typeof(n.cover) !== 'undefined' || n.cover !== null){
              let s = n.cover.split(",")
              for (let i = 0; i < s.length; i++) {
                  this.listCabangCover.push(parseInt(s[i])) 
              }
            } 

            this.ip = n.ip

            if(typeof(n.akses_toko) !== 'undefined' || n.akses_toko !== null){
              let ss = n.akses_toko.split(",")
              for (let i = 0; i < ss.length; i++) {
                 
                  this.akses_toko.push(ss[i]) 
              }
            } 
            ...
            n.flag_gabung === 'Y' ? this.flag_gabung = true : this.flag_gabung = false
            n.ismobile === 'Y' ? this.ismobile = true : this.ismobile = false
            this.overlay = false
          })
          .catch(() => {
            this.overlay = false
