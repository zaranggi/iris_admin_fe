<template>
    ...
    <v-card width="100%" height="100%" elevation="10" class="rounded-lg">
      <v-card-title>  <v-icon color="light-green darken-1">mdi-dns</v-icon>Akses Menu - Iris Admin </v-card-title>
      <v-divider />  
      <v-card-text >   
              ...
              <v-card-text>     
                <v-simple-table>
                   <template v-slot:default>
                    ...
                    <tbody>
                      <tr v-for="i in listmenuadmin" :key="i.id">
                 ...
                 <v-btn small color="teal accent-3" class="d-flex justify-end mb-6" @click="saveMenuAdmin">
                    Simpan
                  </v-btn>
                </v-card-text> 
            </v-card> 
...
export default {
  ...
  computed: {   
    listmenuadmin() {
      return this.$store.getters.listmenuadmin;
    },    
    getonelistmenuadmin() {
      return this.$store.getters.onelistmenuadmin;
    },   
    isMobile() {
  ...
  methods: {  
    loadItemsAdmin() {  
       ...
       const sendpost = { 
        ...
        this.$store.dispatch("getListMenuAdminEdit", sendpost).then(() => {  
          for (let i in this.getonelistmenuadmin) {
            this.selected.push(this.getonelistmenuadmin[i].id);
          }
    ...
    select() {
			this.selected = [];
			if (!this.selectAll) {
				for (let i in this.listmenuadmin) {
					this.selected.push(this.listmenuadmin[i].id);
				}
			}
    }, 
    saveMenuAdmin(){
        ...
        this.$store.dispatch("simpanUpdateMenuAdmin", sendpost).then(() => {
            this.overlay = false
  ...
  mounted() {   
    ...
    this.overlay = true  
    this.$store.dispatch("getOneMenuAdminEdit", sendpost).then(() =>{
      this.overlay = false
      this.loadItemsAdmin()
    }).catch(() => {
      this.overlay = false
