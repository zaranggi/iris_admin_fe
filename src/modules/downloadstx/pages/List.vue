<template>
    ...
    <v-card width="100%" height="100%" elevation="10" class="rounded-lg">
      <v-card-title> Cek Markup WT </v-card-title>
      <v-divider />  
      <v-card-text > 
        <div class="d-flex flex-column ma-4" >
          <v-form @submit.prevent="handleSubmitDownloadstx">
              ...
              <v-row dense justify="center"> 
                <v-col cols="4" xs="12">
                  ...
                  >
                    ...
                    >
                      <v-list-item-action>
                        <v-icon :color="listCabangDownloadstx.length > 0 ? 'indigo darken-4' : ''">
                            {{ iconCabang }}
                          </v-icon>
                      </v-list-item-action>
                      <v-list-item-content>
...
export default {
  ...
  computed: {
    ...
    listdata() {  
      return this.$store.getters.listDataDownloadstx;
    },  
     
    getCabang() {
      return this.$store.getters.listCabangDownloadstx;
    },
    isMobile() {
      return this.windowWidth <= 300
    },
    pilihAllCabang () {
        return this.listCabangDownloadstx.length === this.$store.getters.listCabangDownloadstx.length
    },
    pilihSomeCabang () {
        return this.listCabangDownloadstx.length > 0 && !this.$store.getters.listCabangDownloadstx.length
      },
    iconCabang () { 
  ...
  methods: {  
      handleSubmitDownloadstx(){
        this.overlay = true
        const a = this.listCabangDownloadstx.map((item)=>{ 
        ...
        console.log(datanya)
        this.$store.dispatch("getDownloadstx", datanya).then(()=> {
          this.overlay = false
        }).catch(() => {
          this.overlay = false
        })
      ...
      toggle () {
          this.$nextTick(() => {
            if (this.pilihAllCabang) {
              this.listCabangDownloadstx = []
            } else {
              this.listCabangDownloadstx = this.$store.getters.listCabangDownloadstx.slice()
            }
          })
        },
  ...
  mounted() {   
    ...
    this.$store.dispatch("getCabangDownlaodstx", { kdcab : dataUser.cover});
    this.overlay = false
  },
};
</script>
